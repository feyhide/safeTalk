...
const AuthPage = ({ page, setPageState }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!passwordRegex.test(password)) {
      return toast.error(
        "Password should be 6-20 characters long with at least one numeric digit, one lowercase letter, one uppercase letter, and at least one special character (@#$%^&*!)."
      );
    }
            ...
            <div className="w-full flex gap-2 flex-col items-center justify-center font-slim">
              ...
              >
                {page === "signin"
                  ...
                  : "Already Have Joined ? Just sign in then"}
              </p>
            </div>
          </>
        ) : (
          <>
            <p className="font-heading tracking-tight font-bold text-3xl">
              Verify OTP
            </p>
            <img
              src="/icons/back.png"
              className="absolute left-2 top-5 w-8 h-8"
              onClick={() => {
                setOtp(null), setotpState(false);
              }}
            />
            <div className="w-full flex font-slim flex-col">
