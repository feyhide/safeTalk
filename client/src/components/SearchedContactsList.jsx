...
const SearchedContactsList = ({searchContacts,setSearchContacts}) => {
    ...
    const handleSendConnection = (recipientId) => {
        ...
        setSearchContacts(prevContacts => prevContacts.filter(c => c._id !== recipientId));
    };
        ...
        {searchContacts?.map((contact,index)=>(
            <div key={index} className='w-full flex justify-between items-center py-2'>
                ...
                <div className='w-auto'>
                    <button onClick={() => handleSendConnection(contact._id)} className="p-2 bg-green-500 text-white rounded-xl">Send Connection</button>
                </div>
            </div>
        ))}
    </div>
